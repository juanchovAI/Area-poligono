...
def crear_poli():
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if m1 != m2:  
                    ...
                    print(f"Las aristas {i+1} y {j+1} son coincidentes, lo cual no debería suceder.")
                else:
    ...
    for i in range(0, len(funciones) ): 
        ...
        if(aristas[i][0][0] < aristas[i][1][0]):
            dominio = np.arange(aristas[i][0][0], aristas[i][1][0] + 0.02)
        else:
            dominio = np.arange(aristas[i][1][0], aristas[i][0][0] + 0.02)
